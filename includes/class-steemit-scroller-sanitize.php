...
class Steemit_Scroller_Sanitize {
	...
	public function clean() {
		...
		switch ( $this->type ) {
			...
			case 'radio'			:
			case 'select'			: $sanitized = sanitize_text_field( $this->data ); break;
			...
			case 'array'			: 
				...
				// Loop through the array and sanitize each of the values
				foreach ( $this->data as $key => $val ) 
				{	
					$new_input[ $key ] = ( isset( $this->data[ $key ] ) ) ? sanitize_text_field( $val ) : '';
				}
	...
	public function set_type( $type ) {
		...
		if ( empty( $type ) ) {
			...
			$check = new WP_Error( 'forgot_type', __( 'Specify the data type to sanitize.', 'steemit-scroller' ) );

		}

		if ( is_wp_error( $check ) ) {

			wp_die( $check->get_error_message(), __( 'Forgot data type', 'steemit-scroller' ) );
